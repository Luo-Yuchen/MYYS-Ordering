...
test("包含下单、接单、收款和默认复古怀旧风规则", async () => {
  ...
  assert.match(page, /CloudBase 云端接单/);
...
test("两端通过 CloudBase 云函数遵循自提配送业务规则", async () => {
  ...
  assert.match(cloudFunction, /db.runTransaction/);
  assert.match(cloudFunction, /DELIVERY_MINIMUM = 15/);
  assert.match(cloudFunction, /FREE_DELIVERY_THRESHOLD = 30/);
  assert.match(cloudFunction, /getNextShortCode/);
